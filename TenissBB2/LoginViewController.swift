...
class LoginViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func loginButton() {
        if passwordTextField.text == passwordTextField2.text {
            ...
            ref.child("user").child((user?.uid)!).setValue(["name":usernameTextField.text])
            self.showAlert()
        } else {
            self.showPassAlert()
        }
    ...
    func showAlert() {
        let alert = UIAlertController(title: "登録しました", message:"", preferredStyle: UIAlertControllerStyle.alert)
    ...
    func showPassAlert() {
        let alert = UIAlertController(title: "passwordが入力できていません", message:"", preferredStyle: UIAlertControllerStyle.alert)
